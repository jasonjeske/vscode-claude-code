...
for (const path of learnerFiles) {
  for (const image of read(path).matchAll(/!\[[^\]]*\]\(([^)]+)\)/g)) {
    assert(path === resolve(root, 'README.md') &&
      /^(assets\/readme-banner\.png|assets\/walkthrough\/[a-z0-9-]+\.jpg)$/.test(image[1]),
      `Unreviewed learner image: ${image[1]}`);
  }
