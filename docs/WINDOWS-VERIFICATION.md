...
| W32 | A beginner can follow setup and Lesson 1 through the extension, recognize a pending approval, and find gateway usage without terminal coaching | `guides/00-use-vscode.md`, `START-HERE.md`, `learn/01-first-conversation.md` | Begin on the GitHub document page; download/extract the ZIP, open the correct nested folder, select the two local references, and inspect the setup proposal. Repeat from a wrong/empty folder: no writes and a specific missing-source instruction. Also check unavailable-command fallback, interrupted-output recovery, usage units, and resume note | UNVERIFIED | | Archive/source paths checked locally; no learner or target-PC trial |
