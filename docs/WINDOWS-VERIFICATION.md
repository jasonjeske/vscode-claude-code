...
| W32 | A beginner can follow setup and Lesson 1 through the extension, recognize a pending approval, and find gateway usage without terminal coaching | `guides/00-use-vscode.md`, `START-HERE.md`, `learn/01-first-conversation.md` | Begin on the GitHub page; create the learning location, extract the ZIP, create separate practice subfolders, open the starter in VS Code and reference the guide, switch to practice, save CLAUDE.md, request and inspect the new output or unsaved chat fallback. For optional assisted copying, select its two references and inspect the proposal; repeat from a wrong/empty folder with no writes. Also check approvals, recovery, usage, and resume note | UNVERIFIED | | Archive/source paths checked locally; no learner or target-PC trial |
