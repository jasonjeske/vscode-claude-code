...
|---|---|---|---|---|---|---|
| W01 | `%USERPROFILE%\.claude\CLAUDE.md` is the correct user-level instruction file path | `setup/FULL-ONBOARDING.md` Stage 0, `START-HERE.md` short setup | Open a Windows shell, resolve the path, confirm Claude Code reads instructions from it | UNVERIFIED | | Central to Stage 3; every write target depends on it |
...
| W26 | All six local skills are discovered and invoke correctly in the managed Windows extension | `guides/07-trusted-skills-and-installation.md` | Install one reviewed folder, verify the slash command and behavior, repeat for the others | UNVERIFIED | | No work-PC access; local schema checks are not runtime tests |
...
| W30 | The short setup can copy selected skills and merge global instructions without changing gateway settings | `START-HERE.md`, `setup/SKILLS.md` | In an approved Windows test profile, follow the short path, compare before/after configuration, check discovery and readback | UNVERIFIED | | No target Windows access |
...
| W32 | A beginner can complete the one-page setup in the extension without terminal coaching | `README.md`, `setup/SKILLS.md`, `learn/01-first-conversation.md` | Start on README; open VS Code, create the learning folder, download/extract, create practice subfolders, open Practice-01, save CLAUDE.md, read and copy the two complete skills to personal scope, check both commands and try Prompt Coach, request/open/check the output. Repeat with an existing skill, wrong folder, blocked copy, and missing command; verify no overwrite and clearly pending skill status. Check usage and resume | UNVERIFIED | | Source/folder checks only; no learner or target-PC trial |
